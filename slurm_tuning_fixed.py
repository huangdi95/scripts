...
'''
slurm_file = './run_IO_modular_fixed.slurm'
link_path = './trained_models/tuning_map/'
new_time = time.strftime('%Y%m%d%H%M', time.localtime(time.time()))
csv_file = 'log.tuning_csvs/tuning_IO_modular_fixed_' + new_time + '.csv'
...
params_dict = {}
params_dict.update({'activation': ["'relu'"]})
params_dict.update({'modular.share': ["True"]})
params_dict.update({'modular.hid_dim': [256, 512]})
params_dict.update({'modular.n_layers': [4, 8]})
params_dict.update({'modular.module_pf_dim': ['hid_dim', 'hid_dim * 2', 'hid_dim * 4']})
params_dict.update({'modular.module_heads': [4, 8]})
...
minimum_qos = 'gpu-short'
maximum_qos = 'gpu-short'
if fast_qos:
    fast_qos = 'gpu-short'
else:
    fast_qos = None
...
def main():
    ...
    for idx, values in enumerate(tuning_list):
        ...
        print('idx = %d / %d' % (idx, len(tuning_list)))
