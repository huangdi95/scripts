...
'''
#TODO: config gpu-mode

fast_qos = True 
start_idx = 146

params_dict = {}
params_dict.update({'modular.encode_method': ["'first'", "'mean'"]})
params_dict.update({'activation': ["'gelu'", "'relu'"]})
params_dict.update({'modular.share': ["True", "False"]})
params_dict.update({'modular.hid_dim': [256, 512, 768]})
params_dict.update({'modular.n_layers': [4, 8]})
params_dict.update({'modular.module_pf_dim': ['hid_dim * 2']})
params_dict.update({'modular.enc_layers': [4, 8]})
params_dict.update({'modular.ns': [5, 10]})
params_dict.update({'modular.module_heads': [4]})
params_dict.update({'modular.select_inp_heads': [4]})
params_dict.update({'modular.communication_heads': [4]})
params_dict.update({'modular.enc_heads': [4]})
...
slurm_file = './run_IO_modular.slurm'
params_file = './params.py'
params_indent = ' ' * 4
new_time = time.strftime('%Y%m%d%H%M', time.localtime(time.time()))
csv_file = 'log.tuning_csvs/tuning_IO_modular_' + new_time + '.csv'
...
def submit(min_qos, max_qos):
    config_qos(min_qos, max_qos)
...
def valid_features():
    ...
    for i, f in enumerate(result['features']):
        if 'A100' in f and int(result['remain'][i]) >= 4:
            valid['a100'] = True
        elif 'V100' in f and int(result['remain'][i]) >= 4:
            valid['volta'] = True
        elif 'RTX' in f and int(result['remain'][i]) >= 4:
            valid['rtx8000'] = True
...
def main():
    ...
    for idx, values in enumerate(tuning_list):
        ...
        job_id, saved_dir, saved_dir_all_path = submit(minimum_qos, maximum_qos)
